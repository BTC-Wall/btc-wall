$(document).ready(function() {
	...
	var explorer_addr = "https://www.blockchain.com/explorer/addresses/btc/"
	...
	$("#openBtn").click(function(){
		...
		mnemonic_to_seed(seed).then(valid =>{
          if(valid){
          	var g=temp_account_session_id.split(':');
            address=g[0];
            ...
            for (var key in account_keys_new) {
            	if(account_keys_new[key]['public']==address){
            		var wif=account_keys_new[key]['private'];
            	}
			}
			...
			var pubkey=coinjs.wif2pubkey(wif).pubkey;
			var privkey=coinjs.wif2privkey(wif).privkey;
	...
	$("#walletShowKeys").click(function(){
		...
		$("#walletSpend").removeClass("hidden").addClass("hidden");
                $("#walletSummary").removeClass("hidden").addClass("hidden");
	});
        
        $("#walletShowSummary").click(function(){
		$("#walletSummary").removeClass("hidden");
		$("#walletSpend").removeClass("hidden").addClass("hidden");
			...
			if(dvalue>=total){
				...
				tx2.broadcast(function(data){
					if($(data).find("result").text()=="1"){
						$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-success').html('<big>Sent! Transaction ID://www.blockchain.com/btc/tx/'+$(data).find("txid").text()+'" target="_blank">'+$(data).find("txid").text()+'</a>');
					} else {
	...
	$("#walletSendBtn").click(function(){
		...
		if($("#walletSpend .has-error").length==0){
			...
			$("#walletSendStatus").removeClass("hidden").html('<span class="glyphicon glyphicon-exclamation-sign"></span> Wrong Bitcoin Address or Amount.');
		}
	});
        
        // Handle the click event for the reset button
        $('#walletResetBtn').on('click', function () {
            // Reset the values of input fields
            $('#txtAddress').val(''); // Clear the address field
            $('#input_for_convert').val('0.00'); // Reset the input value for conversion

            // Check and set the minimum value for developerDonation
            var developerDonationValue = parseFloat($('#developerDonation').val().replace(',', '.')) || 0;
            if (developerDonationValue < 0.00015) {
                $('#developerDonation').val('0.00015'); // Set the minimum value
            }

            // Clear the wallet send status and any error indicators
            $('#walletSendStatus').html('').addClass('hidden'); // Hide the status block
            $('.has-error').removeClass('has-error'); // Remove error classes from fields

            // Hide the totalAmount block
            $('#totalAmount').addClass('hidden'); // Add the 'hidden' class to the totalAmount block

            // Recalculate all amounts to ensure proper updates
            recalculateAmounts();
        });
	...
	$("#walletSpendTo .addressAdd").click(function(){
		...
		$('#walletSpendTo .form-horizontal.output:last-child').find('.amount_usd').hide();
		$('#walletSpendTo .form-horizontal.output:last-child').find('.amount_usd').html('$0');
		...
		if($("#walletLoader").hasClass("hidden")){
			...
			coinjs.addressBalance($("#walletAddress").html(),function(data){
				if($(data).find("result").text()==1){
					...
					var usd_balance=(exchange_now*v).toFixed(2);
					$('#walletBalance').next('.balance_usd').html('<br> &#8776; '+usd_balance+' $');
					$('#walletBalance').next('.balance_usd').fadeOut();
					$('#walletBalance').next('.balance_usd').fadeIn();
					
					$("#walletBalance").html(v+" BTC").attr('rel',v).fadeOut().fadeIn();
					$('#walletBalance').attr('data-balance', v);
				} else {
				$("#walletBalance").html("0.00 BTC").attr('rel',v).fadeOut().fadeIn();
				$('#walletBalance').attr('data-balance', 0);

				$('#walletBalance').next('.balance_usd').fadeOut();
				$('#walletBalance').next('.balance_usd').html('$0.00');
				$('#walletBalance').next('.balance_usd').fadeIn();
				}
...
/*	$(".showKey").click(function(){
    ...
    $(".showKey").click(function () {
        ...
        if (input.attr("type") === "password") {
            // Изменить тип на text и текст кнопки на Hide
            input.attr("type", "text");
            $(this).text("Hide");
        } else {
            // Вернуть тип на password и текст кнопки на Show
	...
	function getMouseXY(e) {
        ...
        copyIcon.alt = 'Copy';
        copyIcon.id = 'copyWallet'; // Set ID for the print icon
        copyIcon.style.width = '24px';
        copyIcon.style.marginLeft = '10px';
        copyIcon.style.cursor = 'pointer';


        // Add copy icon after the wallet address
        walletAddressElement.insertAdjacentElement('afterend', copyIcon);

        // Create print icon
        const printIcon = document.createElement('img');
        printIcon.src = 'img/print.png';
        printIcon.alt = 'Print';
        printIcon.id = 'paperWallet'; // Set ID for the print icon
        printIcon.style.width = '24px';
        printIcon.style.marginLeft = '10px';
        printIcon.style.cursor = 'pointer';


        // Add print icon after the copy icon
        copyIcon.insertAdjacentElement('afterend', printIcon);
        ...
        copyIcon.addEventListener('click', (event) => {
            const textToCopy = walletAddressElement.textContent;
        ...
        /* Print paper wallet*/
        document.getElementById('paperWallet').addEventListener('click', function () {
    // Получаем данные кошелька
    const walletAddress = document.getElementById('walletAddress').textContent.trim();
    const seed = document.querySelector('.seed_main').value.trim();
    const publicKey = document.querySelector('.pubkey').value.trim();
    const redeemScript = document.querySelector('.walletSegWitRS input').value.trim();
    const privateKey = document.querySelector('.privkey').value.trim();
    const qrCodeImg = document.querySelector('#walletQrCode img').getAttribute('src'); // Получаем Base64 QR-кода

    // Создаем содержимое для печати
    const printableContent = `
        <html>
        <head>
            <title>Printable Wallet</title>
            <style>
                body {
        ...
        <body>
            ...
            <div class="wallet-detail"><strong>Bitcoin Address:</strong><br/> ${walletAddress}</div>
            <div class="wallet-detail"><strong>Seed:</strong><br/> ${seed}</div>
            <div class="wallet-detail"><strong>Public Key:</strong><br/> ${publicKey}</div>
            <div class="wallet-detail"><strong>Redeem Script (SegWit):</strong><br/> ${redeemScript}</div>
            <div class="wallet-detail"><strong>Private Key:</strong><br/> ${privateKey}</div>
            <div><img src="${qrCodeImg}" alt="QR Code"></div>
            <script>
    ...
    // Открываем новое окно и записываем содержимое
    ...
    printWindow.document.close();
        ...
        // Private key label color
        const walletKeysContainer = document.getElementById("walletKeys");

        if (walletKeysContainer) {
            const fontElements = walletKeysContainer.querySelectorAll("font[color='green']");

        fontElements.forEach((fontElement) => {
            fontElement.style.color = "#212529";
        });
              
        };
        
    
    // Найти блок с id walletQrCode
    const walletQrCodeBlock = $('#walletQrCode');

    if (walletQrCodeBlock.length) {
        // Добавить div с текстом после блока walletQrCode
        walletQrCodeBlock.after('<div style="font-size:14px; font-style: italic; color: #fd8209; padding-bottom: 20px;">Scan your BTC wallet address!</div>');
    } else {
        console.error('Element with id walletQrCode not found');
    }
